...
- **The template is inert.** It sits inside `<x-dc><template>…</template></x-dc>`, so the browser does not fetch the 125 videos and 100+ images it mentions when the page loads; only what the current screen shows is requested. Keep that wrapper when editing `index.html` (the manifest builder and the runtime both understand it).
